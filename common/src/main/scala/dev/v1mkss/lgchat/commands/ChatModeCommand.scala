...
import net.minecraft.server.command.CommandManager.{literal => mcLiteral, argument => mcArgument, RegistrationEnvironment}
...
object ChatModeCommand {
          ...
          .`then`(
            mcArgument("language", StringArgumentType.word())
              .suggests { (ctx, builder) =>
                Language.values.foreach(langEnum => builder.suggest(langEnum.code))
                builder.buildFuture()
              }
  ...
  private def setModeLocal(context: CommandContext[ServerCommandSource]): Int =
    executePlayerCommand(context) { player =>
      ...
      context.getSource.sendFeedback(() => Lang.get(MessageKey.CommandSetPrefixLocal), false)
      1
    }
  ...
  private def setModeGlobal(context: CommandContext[ServerCommandSource]): Int =
    executePlayerCommand(context) { player =>
      ...
      context.getSource.sendFeedback(() => Lang.get(MessageKey.CommandSetPrefixGlobal), false)
      1
    }

  private def showCurrentMode(context: CommandContext[ServerCommandSource]): Int =
    executePlayerCommand(context) { player =>
      val prefersPrefixForGlobal = LGChat.getPlayerPrefersPrefixForGlobal(player.getUuid)
      val feedbackKey = if (prefersPrefixForGlobal) MessageKey.CommandCurrentPrefixGlobal else MessageKey.CommandCurrentPrefixLocal
  ...
  private def setLanguage(context: CommandContext[ServerCommandSource]): Int = {
    ...
    if (Lang.setLang(langArg)) {
      val langDisplayName = Language.fromCode(langArg).map(_.displayName).getOrElse(langArg.toUpperCase)
      val feedback = Lang.getMutable(MessageKey.CommandLangSet).append(
  ...
  private def showCurrentLanguage(context: CommandContext[ServerCommandSource]): Int = {
    val source = context.getSource
    val currentLangDisplay = Lang.getCurrentLanguage.displayName
    val feedback = Lang.getMutable(MessageKey.CommandLangCurrent).append(
